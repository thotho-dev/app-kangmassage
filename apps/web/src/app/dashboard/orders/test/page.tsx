...
import { FlaskConical, Send, MapPin, RefreshCw, CheckCircle, XCircle } from 'lucide-react';
...
export default function TestOrderPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (form.therapist_id) {
        body.therapist_id = form.therapist_id;
      }

      if (selectedService?.price_type === 'duration') {
  ...
  const handlePresetSelect = (index: number) => {
    ...
    if (index === ADDRESS_PRESETS.length - 1) return; // Custom - biarkan user isi manual
    setForm(f => ({
      ...f,
    ...
    <div className="page-container max-w-3xl mx-auto">
      <div className="section-header">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-text-primary">Test Order</h1>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="glass-card p-6 space-y-5">
        ...
        <div>
          ...
          {loading.therapists ? (
            ...
            >
              ...
              {therapists
                .filter((t: TherapistItem) => t.status === 'online' || true)
                .map((t: TherapistItem) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              value={form.therapist_preference}
              onChange={e => setForm(f => ({ ...f, therapist_preference: e.target.value }))}
              className="input-field"
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setForm(f => ({ ...f, user_gender: e.target.value }))}
              className="input-field"
            >
        ...
        <div>
          ...
          <input
            ...
            className="input-field"
            placeholder="Test order dari admin"
          />
        </div>
      ...
      {result && (
        <div className="glass-card p-6 mt-6 border-l-4 border-emerald-500">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-bold text-text-primary">Pesanan Berhasil Dibuat!</h2>
          </div>
      ...
      {/* Error */}
      {error && (
        <div className="glass-card p-6 mt-6 border-l-4 border-danger">
          <div className="flex items-center gap-2 mb-2">
            <XCircle className="w-5 h-5 text-danger" />
            <h2 className="text-lg font-bold text-text-primary">Gagal Membuat Pesanan</h2>
          </div>
